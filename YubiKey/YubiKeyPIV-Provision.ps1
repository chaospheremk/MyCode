# This script is for provisioning a new YubiKey for a new PIV user or a PIV user who needs a replacement YubiKey.
# 
# 1. Enables all applications on the YubiKey.
# 2. Resets all applications on the YubiKey.
# 3. Disables all applications except for PIV.
# 4. Stores the serial number of the YubiKey in a variable called SerialNumber
# 5. Directs the user to set their own PIV PIN.
#
...
# Disable all applications except for PIV over USB and NFC interfaces.
Write-Host "Disabling all applications except for PIV..." -ForegroundColor Yellow
...
Write-Host "All applications except for PIV have been disabled." -ForegroundColor Green
...
Write-Host "PIV YubiKey was successfully provisioned." -ForegroundColor Green
